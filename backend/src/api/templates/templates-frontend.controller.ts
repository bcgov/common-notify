...
export class TemplatesFrontendController {
  ...
  /**
   * List all templates for the selected tenant
   *
   * @param tenantId CSTAR external tenant ID to filter by
   * @param page Page number (1-indexed, default: 1)
   * @param limit Items per page (default: 10, max: 100)
   * @returns Paginated list of templates for the selected tenant
   */
  @Version('1')
  @Get()
  @HttpCode(200)
  @ApiOperation({ summary: 'List all templates for the selected tenant' })
  @ApiQuery({
    name: 'tenantId',
    required: true,
    type: String,
  ...
  async listTemplates(
    ...
    return this.templatesService.listTemplates(tenantExternalId, pageNum, limitNum)
  }

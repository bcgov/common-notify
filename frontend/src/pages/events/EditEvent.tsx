...
const EditEvent: FC<EditEventProps> = ({ eventId, initialTab = 'settings' }) => {
  ...
  async function handleSaveEmailSettings(values: EmailApplyValues) {
    const updated = await updateEventEmailSettings(eventId, {
      ...
      cc: values.cc,
      bcc: values.bcc,
      cstarGroupIdsTo: values.cstarGroups.to,
      cstarGroupIdsCc: values.cstarGroups.cc,
      cstarGroupIdsBcc: values.cstarGroups.bcc,
        ...
        {loadError ? (
          ...
          <EventsEmailTab
            values={{
              ...
              cstarGroups: {
                to: event.emailSettings?.cstarGroupIdsTo ?? [],
                cc: event.emailSettings?.cstarGroupIdsCc ?? [],
                bcc: event.emailSettings?.cstarGroupIdsBcc ?? [],
              },
